...
Write-Host "[1/5] Preparing installation files..." -ForegroundColor Yellow
...
Write-Host "[2/5] Creating application metadata..." -ForegroundColor Yellow
...
Write-Host "[3/5] Creating installation script..." -ForegroundColor Yellow

# Create installation script using separate file approach
...
set "SHORTCUT_TARGET=%TARGET_DIR%\$AppExe"
set "SHORTCUT_FILE=%USERPROFILE%\Desktop\$AppName.lnk"

powershell -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('%SHORTCUT_FILE%'); $Shortcut.TargetPath = '%SHORTCUT_TARGET%'; $Shortcut.WorkingDirectory = '%TARGET_DIR%'; $Shortcut.Description = '$AppName v$Version'; $Shortcut.Save()"
...
set "START_SHORTCUT_FILE=%START_MENU_DIR%\$AppName.lnk"
powershell -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('%START_SHORTCUT_FILE%'); $Shortcut.TargetPath = '%SHORTCUT_TARGET%'; $Shortcut.WorkingDirectory = '%TARGET_DIR%'; $Shortcut.Description = '$AppName v$Version'; $Shortcut.Save()"
...
[System.IO.File]::WriteAllText($InstallScriptPath, $InstallScriptContent, [System.Text.Encoding]::ASCII)
Write-Host "  ✓ Created installation script" -ForegroundColor Green

Write-Host "[4/5] Creating uninstaller..." -ForegroundColor Yellow
...
[System.IO.File]::WriteAllText($UninstallScriptPath, $UninstallScriptContent, [System.Text.Encoding]::ASCII)
