"""
Build Metrology Workstation Installer with CALIBRA Icon
"""
...
def build_installer():
    print("=" * 70)
    print(" BUILDING METROLOGY WORKSTATION INSTALLER WITH CALIBRA ICON")
    print("=" * 70)
    os.makedirs(DIST_WORKSTATION_DIR, exist_ok=True)

    # Check for CALIBRA icon
    icon_path = os.path.join(PROJECT_ROOT, "assets", "calibra_icon.ico")
    ...
    print("Running PyInstaller compilation with CALIBRA icon...")
    ...
    if os.path.exists(built_exe):
        ...
        print("\n" + "=" * 70)
        print(" CALIBRA BRANDED INSTALLER BUILT SUCCESSFULLY")
        print(f" Output Location: {TARGET_EXE}")
        print(f" File Size:       {size:,} bytes ({size / (1024*1024):.2f} MB)")
        print(f" Icon:           CALIBRA branding")
        print("=" * 70)
    else:
